...
class PropertyController extends Controller
{
    ...
    {
        // Check permission
        if (!auth()->user()->can('properties.create') && !auth()->user()->can('properties.manage')) {
            return response()->json(['message' => 'You do not have permission to create properties'], 403);
        }
    ...
    {
        // Check permission
        if (!auth()->user()->can('properties.edit') && !auth()->user()->can('properties.manage')) {
            return response()->json(['message' => 'You do not have permission to edit properties'], 403);
        }
    ...
    {
        // Check permission
        if (!auth()->user()->can('properties.delete') && !auth()->user()->can('properties.manage')) {
            return response()->json(['message' => 'You do not have permission to delete properties'], 403);
        }
        $property = Property::find($id);
        if (!$property) {
            return response()->json(['message' => 'Property not found'], 404);
